...
def load_eggs(areaid, path='.'):
    ...
    items = list(Item(to_position(pos), areaid, EGG_ID) for pos, eventid in enumerate(tiledata) if eventid == EGG_EVENT_ID)
    f.close()
    return list(items)

def load_items(areaid, path='.'):
    f = open(map_filename(areaid, path), 'rb')
...
def item_modification():
    grab_original_maps('test_maps')
    areaids = list(range(10))
    mod = ItemModifier(areaids, source_dir='original_maps')
